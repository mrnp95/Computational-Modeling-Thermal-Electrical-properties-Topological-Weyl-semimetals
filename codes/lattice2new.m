...
til=linspace(0,5,n);           %tilting parameter
%nernstp=zeros(n,1);
sigl=zeros(n,1);
alpl=zeros(n,1);
sigt=zeros(n,1);
...
%delHkx
delHkx(1,1,:,:,:)=2*t*cos(kx);
delHkx(2,1,:,:,:)=-2*t*sin(kx);
delHkx(1,2,:,:,:)=-2*t*sin(kx);
delHkx(2,2,:,:,:)=-2*t*cos(kx);
