...
H(1,1,:,:,:)=gm*sin(kx) + 2*t*sin(kx);
H(2,1,:,:,:)=2*t*(cos(kx) + cos(ky) - 2) - t*sin(ky)*2i - 2*t*cos(kz);
H(1,2,:,:,:)=2*t*(cos(kx) + cos(ky) - 2) + t*sin(ky)*2i - 2*t*cos(kz);
H(2,2,:,:,:)=gm*sin(kx) - 2*t*sin(kx);
...
%delHkx
delHkx(1,1,:,:,:)=gm*cos(kx) + 2*t*cos(kx);
delHkx(2,1,:,:,:)=-2*t*sin(kx);
delHkx(1,2,:,:,:)=-2*t*sin(kx);
delHkx(2,2,:,:,:)=gm*cos(kx) - 2*t*cos(kx);
...
thetaP=alpt./alpl;
thetaH=sigt./sigl;
nernst=(alpl./sigl).*(thetaP-thetaH);
gmp=til;
%{
Bp=zeros(2*noofB,1);
