...
kbolz=8.61733*10^-5;
beta=1/(kbolz*T);
t=.05;                     %lattice parameter

mu=0.05;                    %chemical potential
...
n=30;                          %no of values of gamma 
til=linspace(0,3,n);           %tilting parameter
%nernstp=zeros(n,1);
sigl=zeros(n,1);
alpl=zeros(n,1);
sigt=zeros(n,1);
...
H(1,1,:,:,:)=gm*cos(2*kz).*cos(kx) - 2*t*cos(kx);
H(2,1,:,:,:)=2*t*(cos(kx).^2 + cos(ky) - 1) - t*cos(kz) - t*sin(ky)*2i;
H(1,2,:,:,:)=2*t*(cos(kx).^2 + cos(ky) - 1) - t*cos(kz) + t*sin(ky)*2i;
H(2,2,:,:,:)=2*t*cos(kx) + gm*cos(2*kz).*cos(kx);
...
%delHkx
delHkx(1,1,:,:,:)=2*t*sin(kx) - gm*cos(2*kz).*sin(kx);
delHkx(2,1,:,:,:)=-4*t*cos(kx).*sin(kx);
delHkx(1,2,:,:,:)=-4*t*cos(kx).*sin(kx);
delHkx(2,2,:,:,:)=- 2*t*sin(kx) - gm*cos(2*kz).*sin(kx);
